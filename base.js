...
// --- BANCO DE DADOS SIMULADO (Simulando o que seria baixado para o modo offline) ---
const VAGAS_MOCK = [
...
export default function Index() {
  const [abaAtual, setAbaAtual] = useState('vagas'); // Controle de navegação
  const [nome, setNome] = useState('');
  const [habilidadesUsuario, setHabilidadesUsuario] = useState('');
  const [vagas, setVagas] = useState([]);
  const [cursos, setCursos] = useState([]);

  // Simulando a inicialização do app carregando os dados locais (offline-first)
  useEffect(() => {
    setVagas(VAGAS_MOCK);
        ...
        {abaAtual === 'curriculo' && (
          <ScrollView style={{ flex: 1 }}>
            ...
            <TextInput 
              ...
              placeholder="Ex: Boa comunicação, pontual, facilidade com computador..." 
              multiline={true}
...
// --- ESTILIZAÇÃO DO APP (Focado em leveza e alto contraste para acessibilidade) ---
const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: '#F5F7FA', // Fundo claro e limpo
  },
  header: {
    backgroundColor: '#2E5BFF', // Azul vibrante, passa profissionalismo
    padding: 20,
    alignItems: 'center',
  },
  headerTitle: {
  ...
  infoInclusao: {
    fontSize: 12,
    color: '#059669', // Verde para destacar a ação social positiva
    marginBottom: 15,
    fontWeight: '600',
  },
  card: {
